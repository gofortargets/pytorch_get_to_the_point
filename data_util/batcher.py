...
# init bert
print ('Initing bert tokenizer')
bert_tokenizer = BertTokenizer.from_pretrained(config.bert_model, do_lower_case=config.do_lower_case)

def make_bert_input(original_text: str, max_len) -> (List[int], List[int]):
    global bert_tokenizer
    # all_input_ids = []
    # all_input_mask = []
    # for example in original_text:
    original_tokens = bert_tokenizer.tokenize(original_text)
    if len(original_tokens) > max_len:
        original_tokens = original_tokens[:max_len]
    assert len(original_tokens) <= max_len
    ...
    while len(input_ids) < max_len + 2:
        input_ids.append(0)
        input_mask.append(0)

    # [CLS] & [SEQ] token
    assert len(input_ids) == max_len + 2
    assert len(input_mask) == max_len + 2
...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab):
    ...
    # bert encode
    self.enc_bert_input, self.enc_bert_input_mask = make_bert_input(article, config.max_enc_steps)
    # self.enc_bert_embeded = make_bert_embedded(self.enc_bert_input, self.enc_bert_input_mask,
    ...
    # bert decode
    self.dec_bert_input, self.dec_bert_input_mask = make_bert_input(abstract, config.max_dec_steps)
    self.include_end = True
    if len(abstract_words) > config.max_dec_steps:
      self.include_end = False
    # self.dec_bert_embeded = make_bert_embedded(self.dec_bert_input, self.dec_bert_input_mask,
    #                                            config.max_dec_steps, include_start=True,
    ...
    # print ('Finish 1 example in', end - start)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list : List[Example]):
    ...
    # bert embedding
    # self.enc_bert_embeded = torch.cat([ex.enc_bert_embeded for ex in example_list], dim=0)
      self.enc_bert_input = np.stack([ex.enc_bert_input for ex in example_list])
      self.enc_bert_mask = np.stack([ex.enc_bert_input_mask for ex in example_list])
  ...
  def init_decoder_seq(self, example_list: List[Example]):
    ...
    # bert embedding
    # self.dec_bert_embeded = torch.cat([ex.dec_bert_embeded for ex in example_list], dim=0)
      self.dec_bert_input = np.stack([ex.dec_bert_input for ex in example_list])
      self.dec_bert_mask = np.stack([ex.dec_bert_input_mask for ex in example_list])
